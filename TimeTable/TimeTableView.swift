                                        ...
                                        if(activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0).count == 0) {
                                            ...
                                            ForEach(0..<activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0).count, id: \.self) { index in
                                                if(date.weekDay - 1 != dayIndex) {
                                                    Card(state: .constant(CardState.disable), lesson: .constant(activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0)[index]), date: $date)
                                                } else {
                                                    let state: CardState = date.minutes > activeTimeTable.info.days[selectedDay].getLessons(date: date, index: 0)[index].end ? .disable : (date.minutes >= activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0)[index].start && date.minutes <= activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0)[index].end) ? .active : (date.minutes < activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0)[index].start && (index == 0 || date.minutes > activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0)[index - 1].end)) ? .wait : .highlight
                                    ...
                                    VStack {
                                        if(activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 0).count == 0) {
                                            ...
                                            ForEach(0..<activeTimeTable.info.days[dayIndex].getLessons(date: date, index: 1).count, id: \.self) { index in
                                                Card(state: .constant(CardState.disable), lesson:
